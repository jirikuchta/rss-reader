...
async function build$2() {
    clear(node$3);
    items = [];
    let scroll = node$4("div", { className: "scroll" }, "", node$3);
    list$1().forEach(c => scroll.appendChild(buildCategory(c)));
    let uncategorized = node$4("ul", {}, "", scroll);
    list$2()
    ...
    let footer = node$4("footer", {}, "", node$3);
...
function buildCategory(category) {
    ...
    categoryItem.node.classList.toggle("is-collapsed", (getItem("collapsedCategories") || []).includes(category.id));
...
class Item {
    ...
    updateCounter() {
        ...
        if (this.type == "category") {
            ...
            count = get$3(this.id) || 0;
        }
        clear(this.counter);
        count && this.counter.appendChild(text(`${count}`));
    }
}
function toggleCategory(item) {
    item.node.classList.toggle("is-collapsed");
    let data = getItem("collapsedCategories") || [];
    ...
    setItem("collapsedCategories", data);
}
function showItemPopup(item, e) {
    e.preventDefault();
...
function init$3() {
    ...
    subscribe("articles-read", async () => {
        let nodes = node$2.querySelectorAll(`article:not(.${READ_CSS_CLASS})`);
        for (let n of nodes) {
            let article = await get(parseArticleId(n));
            (article === null || article === void 0 ? void 0 : article.read) && n.classList.add(READ_CSS_CLASS);
        }
    });
...
async function build$1() {
    ...
    data.forEach(article => {
        ...
        elm && node$2.appendChild(elm);
    });
    showMoreObserver.disconnect();
    data.length && showMoreObserver.observe(node$2.querySelector("article:last-of-type"));
}
function buildItem(article) {
    let subscription = get$2(article.subscription_id);
    if (!subscription) {
        return;
    }
    ...
    elm.appendChild(node$4("h3", {}, article.title));
    elm.appendChild(node$4("p", {}, article.summary));
...
function select(n, focus = false) {
    var _a;
    (_a = getSelectedNode()) === null || _a === void 0 ? void 0 : _a.classList.remove(SELECTED_CSS_CLASS);
...
function getSelectedNode() {
    let elm = node$2.querySelector(`.${SELECTED_CSS_CLASS}`);
    return elm ? elm : null;
}
function getSibling(dir) {
    let n = getSelectedNode();
    if (!n) {
        return dir == "next" ? node$2.firstElementChild : null;
    }
    return (dir == "next" ? n.nextElementSibling : n.previousElementSibling);
}
function parseArticleId(elm) {
