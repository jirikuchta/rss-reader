...
document.addEventListener("mousedown", e => current$1 === null || current$1 === void 0 ? void 0 : current$1.close());
...
async function build$2() {
    clear$1(node$3);
    items$1 = [];
    list$1().forEach(c => node$3.appendChild(buildCategory$1(c)));
    ...
    let add = button({ icon: "plus" }, "", footer);
...
function showItemPopup(item, e) {
    ...
    if (item.type == "subscription") {
        ...
        menu.addItem("Edit subscription", "pencil", () => editSubscription(item.data));
        menu.addItem("Unsubscribe", "trash", () => deleteSubscription(item.data));
    }
    else {
        menu.addItem("Mark as read", "check-all", () => markRead$1(item.data.id));
        menu.addItem("Edit category", "pencil", () => editCategory(item.data));
...
let items = [];
let selectedIndex = -1;
const observer = new IntersectionObserver(entries => entries[0].isIntersecting && build$1(), { root: node$2, rootMargin: "0% 0% 33% 0%", threshold: 1 });
function init$3() {
    build$1();
    subscribe("nav-item-selected", _ => clear() && build$1());
...
async function build$1() {
    ...
    data.forEach((article, i) => node$2.appendChild(buildItem(article)));
...
function select(index, scrollIntoView = false) {
    var _a;
    let articleNode = node$2.querySelector(`article:nth-of-type(${index + 1})`);
    if (!articleNode) {
        return;
    }
...
function parse(opml) {
    ...
    return nodes.map(node => {
        ...
        return ({
            ...
            category: ((_a = node.closest("body > outline")) === null || _a === void 0 ? void 0 : _a.getAttribute("title")) || null
        });
    });
}
function serialize() {
...
function buildCategory(doc, data) {
