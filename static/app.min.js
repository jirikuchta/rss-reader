...
async function sync() {
    ...
    for (const [key, value] of Object.entries(res.data)) {
        counters.set(Number(key), value);
    }
    publish("subscriptions-changed");
}
function get(id) { return counters.get(id); }
...
async function build$2() {
    ...
    list$2().forEach(s => {
        s.category_id === null && uncategorized.appendChild(buildItem$1(s));
    });
}
function buildCategory(category) {
    ...
    list$2().forEach(s => {
        s.category_id == category.id && list.appendChild(buildItem$1(s));
    });
    return list;
}
function buildItem$1(entity) {
    ...
    if (isSubscription(entity)) {
        ...
        let count = get(entity.id);
        count && node.appendChild(node$4("span", { className: "count" }, `${count}`));
    }
    else {
