...
let selected$1;
const observer = new IntersectionObserver(loadMore, { root: node$2, threshold: 1 });
...
async function ble() {
    ...
    if (items) {
        let lastItem = Array.from(node$2.querySelectorAll("article")).pop();
        lastItem && observer.unobserve(lastItem);
        items.forEach(article => node$2.appendChild(buildItem$1(article)));
        lastItem = Array.from(node$2.querySelectorAll("article")).pop();
