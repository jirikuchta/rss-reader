...
class CategoryItem extends Item$1 {
    ...
    build() {
        super.build();
        let opener = button({ icon: "chevron-down", className: "plain btn-chevron" });
...
class SubscriptionItem extends Item$1 {
    ...
    updateCounter() {
        clear(this._counter);
        let count = get$3(this.data.id) || 0;
        count && this._counter.appendChild(text(`${count}`));
    }
    build() {
        super.build();
        this.node.insertAdjacentElement("afterbegin", build$3(this.data));
    }
}
