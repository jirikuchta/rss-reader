...
class SubscriptionForm {
    ...
    _build() {
        ...
        if (this._subscription) {
            ...
            let catTitle = (_a = get$1()
                .find(c => { var _a; return c.id == ((_a = this._subscription) === null || _a === void 0 ? void 0 : _a.category_id); })) === null || _a === void 0 ? void 0 : _a.title;
...
function buildCategoryList() {
    ...
    get$1().forEach(c => node$4("option", { value: c.title }, c.title, node));
    return node;
}
...
function buildCategory(category) {
    let list = node$4("ul");
    list.appendChild(buildItem$1(category));
    get$2()
        .filter(s => s.category_id == category.id)
        .forEach(s => list.appendChild(buildItem$1(s)));
    return list;
}
function buildItem$1(entity) {
    let node = node$4("li");
    if (isSubscription(entity)) {
        node.appendChild(build$3(entity));
        node.appendChild(node$4("span", { className: "title" }, entity.title));
        node.appendChild(node$4("span", { className: "count" }, "50"));
    }
    else {
    ...
    node.addEventListener("click", e => selectItem$1(node, entity));
    let btn = button({ className: "plain btn-dots", icon: "dots-horizontal" }, "", node);
    btn.addEventListener("click", e => {
        e.stopPropagation();
...
async function buildItems() {
    observer.disconnect();
    let items = await get(get_filters());
    if (items.length) {
