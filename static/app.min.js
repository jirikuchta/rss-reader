    ...
    _validate(res) {
        ...
        switch ((_a = res.error) === null || _a === void 0 ? void 0 : _a.code) {
            ...
            case "ambiguous_feed_url":
                this._feedSelect = node("select");
                node("option", { value: this._url.value }, "Select feed…", this._feedSelect);
                res.error.links.forEach(l => node("option", { value: l.href }, l.title || l.href, this._feedSelect));
...
function buildCategory(category) {
    let list$1 = node("ul");
    list$1.appendChild(buildItem(category));
    list()
        .filter(s => s.category_id == category.id)
        .forEach(s => list$1.appendChild(buildItem(s)));
    return list$1;
}
function buildItem(entity) {
    let node$1 = node("li");
    if (isSubscription(entity)) {
        node$1.appendChild(build(entity));
...
function selectItem(itemNode, entity) {
    var _a;
    (_a = node$1.querySelector(`.${SELECTED_CSS_CLASS}`)) === null || _a === void 0 ? void 0 : _a.classList.remove(SELECTED_CSS_CLASS);
...
const node$2 = node("section", { "id": "list" });
let selected$1;
...
function init$3() {
    build$2();
    subscribe("nav-item-selected", build$2);
}
...
async function buildItems() {
    observer.disconnect();
    let items = await list$2(get_filters());
    if (items.length) {
        items.forEach(article => node$2.appendChild(buildItem$1(article)));
        observer.observe(node$2.querySelector("article:last-child"));
    }
}
function buildItem$1(article) {
    let subscription = get(article.subscription_id);
    let node$1 = node("article");
    node$1.addEventListener("click", _ => selectItem$1(node$1, article));
    let header = node("header", {}, "", node$1);
    header.appendChild(build(subscription));
    header.appendChild(node("h6", {}, subscription.title));
    header.appendChild(node("time", {}, date(article.time_published)));
...
function get_filters() {
    ...
    if (selected) {
        if (isSubscription(selected)) {
            filters["subscription_id"] = selected.id;
        }
        else {
            filters["category_id"] = selected.id;
        }
    }
    return filters;
}
async function selectItem$1(itemNode, article) {
    var _a;
    (_a = node$2.querySelector(`.${SELECTED_CSS_CLASS$1}`)) === null || _a === void 0 ? void 0 : _a.classList.remove(SELECTED_CSS_CLASS$1);
...
const node$3 = node("section", { "id": "detail" });
function init$4() {
    build$3();
    subscribe("article-selected", build$3);
}
async function build$3() {
    ...
    node("a", { href: selected$1.url, target: "_blank", rel: "noopener noreferrer" }, selected$1.title, title);
    let content = node("div", {}, "", frag);
    content.innerHTML = selected$1.content || selected$1.summary || "";
    node$3.appendChild(frag);
    toggle_read(selected$1);
}
...
async function init$5() {
    ...
    init$3();
    init$4();
    node$4.appendChild(node$1);
    node$4.appendChild(node$2);
    node$4.appendChild(node$3);
    new Resizer(node$1, "sidebar-width");
